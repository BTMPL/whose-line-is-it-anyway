...
def format_message(comment):
    ...
    nick = '{name}'.format(badges=badges, name=name, c=color)    
    return "[{time}] [{nick}] {message}".format(time=time_to_hhmmss(time), nick=nick, message=message)
...
cursor = None
contentOffsetSeconds = None
if data[0]['data']['video']['comments']['pageInfo']['hasNextPage']:
    cursor = data[0]['data']['video']['comments']['edges'][-1]['cursor']
    contentOffsetSeconds = data[0]['data']['video']['comments']['edges'][-1]['node']['contentOffsetSeconds']
    time.sleep(0.1)

while contentOffsetSeconds:
    
    response = session.post(
        'https://gql.twitch.tv/gql',
        "[{\"operationName\":\"VideoCommentsByOffsetOrCursor\"," +
            "\"variables\":{\"videoID\":\"" + video_id + "\",\"contentOffsetSeconds\":" + str(contentOffsetSeconds) + "}," +
            "\"extensions\":{\"persistedQuery\":{\"version\":1,\"sha256Hash\":\"b70a3591ff0f4e0313d126c6a1502d79a1c02baebb288227c582044aa76adf6a\"}}}]",
        timeout=10)
    response.raise_for_status()
    data = response.json()
